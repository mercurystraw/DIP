...
# 计算配准图像和原始图像的差异并保存
def save_difference_image(original_image_array, registered_image_array, filename):
    # 确保输入的图像是 NumPy 数组
    original_image_array = np.array(original_image_array)
    registered_image_array = np.array(registered_image_array)

    # 计算差异图像
    difference_image = np.abs(original_image_array.astype(np.int16) - registered_image_array.astype(np.int16))
    difference_image = np.clip(difference_image, 0, 255).astype(np.uint8)  # 确保像素值在合法范围内

    # 保存差异图像
    difference_image_pil = Image.fromarray(difference_image)
    difference_image_pil.save(filename)
